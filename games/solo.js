...
async function rollDice(message) {
    ...
    const botRoll = Math.floor(Math.random() * 6) + 1;
    const win = userRoll > botRoll;
    if (win) addReward(message.author, rewards.soloCoins, rewards.soloXp, "soloWins");
    return message.reply({ embeds: [new EmbedBuilder().setColor(win ? "#2ECC71" : userRoll === botRoll ? color : "#E74C3C").setTitle("🎲 رمي النرد").setDescription(`أنت رميت: **${userRoll}**\nونسة رمى: **${botRoll}**\n\n${win ? "✅ فزت!" : userRoll === botRoll ? "تعادل!" : "❌ خسرت."}`)] });
}
